        ...
        while True:
            if self.computer and self.model.get_current_player() == "O":
                ...
                self.view.display_message(
                    f"Computer Player chose ({row}, {column})")
            else:
                row = int(self.view.get_input(
                    f"Player {self.model.get_current_player()}, enter row: "))
                column = int(self.view.get_input(
                    f"Player {self.model.get_current_player()}, enter column: "))
            ...
            if self.model.update_board(row, column):
                ...
                if self.model.check_winner(self.model.get_current_player()):
                    self.view.display_message(
                        f"Player {self.model.get_current_player()} is the winner!")
                    break
                self.model.toggle_player()
            else:
                self.view.display_message(
                    'Please enter different coordinates or position already occupied')
